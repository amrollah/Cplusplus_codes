...
void ClientFramework::parseArguments (int argc, char *argv[]) {
	const int MAX_COUNT_OF_ARGS = 7;
	const int COUNT_OF_OPTIONAL_ARGS = 1;
...
bool ClientFramework::connectToServer () {
	...
	std::cout << "Connecting to Partov server at " <<
		(ip >> 24) << "." <<
		((ip >> 16) & 0xFF) << "." <<
		((ip >> 8) & 0xFF) << "." <<
		(ip & 0xFF) << ":" << ntohs (server.sin_port) << std::endl;
	std::cout << "           to map/node \"" <<
		mapName << "\"/\"" << nodeName << "\" ... " << std::flush;
...
bool ClientFramework::doMapSelectingNegotiations (byte *buf) {
	...
	if (3 * sizeof (uint16) + mapName.length () + creatorId.length ()
		+ sizeof (int32) > FRAME_BUFFER_SIZE) {
		std::cout << "[Failed]" << std::endl;
	...
	do {
		...
		case MapNotExistCommand:
			std::cout << "[Failed]" << std::endl;
			std::cout << "+++ Initial negotiations failed (map not exists). [error code 5]" << std::endl;
			return false;

		case DuplicateMapIdCommand:
			std::cout << "[Failed]" << std::endl;
			std::cout << "+++ Initial negotiations failed (duplicate map id). [error code 6]" << std::endl;
			return false;
		...
		case MapInstanceResourcesAreReleased:
			std::cout << "[Done]" << std::endl;
			std::cout << "+++ Whole of the resources of the map instance are released." << std::endl;
			return false;

		case OutOfResourceCommand:
			std::cout << "[Failed]" << std::endl;
			std::cout << "+++ Initial negotiations failed (out of resource; retry later). [error code 7]" << std::endl;
			return false;
...
bool ClientFramework::doNodeSelectingNegotiations (byte *buf) {
	if (nodeName == "") {
		...
		std::cout << "No node name specified. So we let map work independently." << std::endl;
	...
	do {
		...
		case NodeNotExistCommand:
			std::cout << "[Failed]" << std::endl;
			std::cout << "+++ Initial negotiations failed (node not exists). [error code 12]" << std::endl;
			return false;
...
bool ClientFramework::doInterfacesInformationSynchronizationNegotiations (byte *buf) {
	...
	do {
		...
		if (c1 != InformationSynchronizationType || c2 != InterfacesInformationSynchronizationSubType || count < 1) {
			std::cout << "[Failed]" << std::endl;
...
bool ClientFramework::readInterfaceInformation (byte *buf, int index) {
	...
	if (strcmp (ifName, "ir::hvns::nss::map::interface::EthernetInterface") != 0
		&& strcmp (ifName, "ir::hvns::nss::map::interface::EthernetPhysicalInterface") != 0) {
		std::cout << "[Failed]" << std::endl;
		std::cout << "+++ Initial negotiations failed (unknown interface type). [error code 19]" << std::endl;
		return false;
	}
...
bool ClientFramework::doCustomInformationSynchronizationNegotiations (byte *buf) {
	...
	do {
		...
		if (c1 != InformationSynchronizationType || c2 != CustomInformationSynchronizationSubType || size < 1) {
			std::cout << "[Failed]" << std::endl;
...
bool ClientFramework::startSimulation () {
	...
	std::cout << "[Done]" << std::endl;
	std::cout << "Simulation started. You will receive all frames of the machine from now on." << std::endl;
	std::cout << "==========================================" << std::endl;
...
void ClientFramework::simulateMachine () {
	...
	if (pthread_create (&stid, NULL, &ClientFramework::st_run, machine)) {
		...
		std::cout << "Can not start the simulator thread. Check POSIX installation." << std::endl;
		return;
	}
	...
	forever {
		if (!sendOrReceive (false, -1, sizeof (uint32) + sizeof (uint16))) {
			std::cout << "An IO exception occurred (error code 26). Aborting simulation..." << std::endl;
			break;
		}
		byte *buf = buffer;
		...
		if (c == InvalidInterfaceIndexType) {
			if (size < 1 || !sendOrReceive (false, -1, size)) {
				std::cout << "An IO exception occurred (error code 27). Aborting simulation..." << std::endl;
				break;
			}
			const char *desc = reinterpret_cast<char *> (buffer);
			std::cout << "Error in sending frame on simulated machine (interface index was wrong)." << std::endl;
			std::cout << "-- " << desc << std::endl;
			continue;
		}
		if (c != RawFrameReceivedNotificationType || size < sizeof (uint32) || size > FRAME_BUFFER_SIZE) {
			std::cout << "An IO exception occurred (error code 28). Aborting simulation..." << std::endl;
			break;
		}
		if (!sendOrReceive (false, -1, size)) {
			std::cout << "An IO exception occurred (error code 29). Aborting simulation..." << std::endl;
			break;
		}
		buf = buffer;
		...
		if (interfaceIndex < 0 || interfaceIndex >= machine->getCountOfInterfaces ()) {
			std::cout << "An IO exception occurred (error code 30). Aborting simulation..." << std::endl;
			break;
		}
		size -= sizeof (uint32);
...
bool ClientFramework::realSendFrame (Frame frame, int ifaceIndex) const {
	...
	if (frame.length < 14 || frame.length >= ((1 << 16) - sizeof (uint32))) {
		std::cout << "Error in sending frame on simulated machine. [error code 31]" << std::endl;
		return false;
	}
	...
	if (!sendOrReceive (true, -1, sizeof (mybuf), mybuf)) {
		std::cout << "Error in sending frame on simulated machine. [error code 32]" << std::endl;
		return false;
	}
	if (!sendOrReceive (true, -1, frame.length, frame.data)) {
		std::cout << "Error in sending frame on simulated machine. [error code 33]" << std::endl;
		return false;
	}
...
bool ClientFramework::sendOrReceive (bool sendIt, int errorCode, int size, byte *buf) const {
	int count = 0;
	do {
		size -= count;
		buf += count;
		...
		if (count < 0) {
			...
			std::cout << "[Failed]" << std::endl;
			std::cout << "+++ Initial negotiations failed. [error code " << errorCode << "]" << std::endl;
			return false;
		}
	} while (count < size);
	return true;
...
void ClientFramework::usage () const {
	std::cout << "Copyright (C) 2009-2010 Behnam Momeni. All rights reserved." << std::endl;
	std::cout << "HVNS Project - Version 2.0 -- Client Framework" << std::endl;
	...
	std::cout << "                                    Any user can only create (at maximum) one instance of each map." << std::endl;
